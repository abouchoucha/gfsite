...
function calcTimeOffset(d, timezoneoffset) {
	calctime = d.getTime() - ((timezoneoffset + (new Date().getTimezoneOffset()/60))*3600000);
...
function getCurrentInitTime(offset) {
	return calcTime(getCurrentTimeHHmmss(new Date(), "0", "0", "0"),offset);
}

function getCurrentEndTime(offset) {
	return calcTime(getCurrentTimeHHmmss(new Date(), "23", "59", "59"),offset);
}

function getCurrentTimeHHmmss(currentDate, hours, minutes, seconds) {
...
function CT_ToggleTimeZones(timeId) {
    ...
    obj = document.getElementById(timeId);
    ...
    if (obj != null) {
		...
		*/
        ...
        if (iTime == 1)
            if (bLocal)*/
                var eTime = document.getElementById(timeId);
