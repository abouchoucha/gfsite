...
class Player extends Zend_Db_Table_Abstract {
	...
	public function getPlayersForSearchResult($playerId) {
    	$db = $this->getAdapter () ;
        $sql = " SELECT ";
		$sql .= " p.player_id, ";
   ...
   public function getPlayerTeamTotalStatsDetails ($playerId,$format) {
    ...
    public function getPlayerKeeperTeamStatsDetails ($playerId,$format) {
        $db = $this->getAdapter () ;
        $sql = " select p.player_name_short,p.player_common_name,tp.team_id,tp.team_name,t.team_seoname,tp.season_id,tp.season_name,lc.competition_id,lc.competition_name,tp.games_played AS gp, tp.substitute, tp.minutes, tp.wins, tp.draws, tp.loses, tp.goals_allowed AS ga,";
		$sql .= " tp.goals_against_average, tp.clean_sheets, tp.shots_allowed, tp.save_percentage, tp.save_per_90_min, tp.yellow_cards AS yc, tp.red_cards AS rc ";
 ...
 public function getPlayerKeeperTeamTotalStatsDetails ($playerId,$format) {
        $db = $this->getAdapter () ;
        $sql = " SELECT p.player_name_short,p.player_common_name,tp.team_id,tp.team_name,t.team_seoname,tp.season_id,tp.season_name,lc.competition_id,lc.competition_name,sum(tp.games_played) AS total_gp,sum(tp.goals_allowed) AS total_gl,sum(tp.yellow_cards) AS total_yc,sum(tp.red_cards) AS total_rc";		
        $sql .= " from teamgoaliestats tp,team t,player p,league_competition lc" ;
		$sql .= " where tp.player_id = ".$playerId ;
        $sql .= " and p.player_id = tp.player_id ";
        $sql .= " and tp.team_id = t.team_id ";
...
public function findFeaturedPlayers($count = null , $regiongroupid = null)
	{
        $db = $this->getAdapter () ;
        ...
        if(!is_null($count)){
        	$sql .= "  order by fp.sort_order limit 0, ".$count;
        }
        //echo $sql;
        $result = $db->query ( $sql ) ;
		$row = $result->fetchAll () ;
	...
	public function findPopularPlayers($count)
	{
		$db = $this->getAdapter();
