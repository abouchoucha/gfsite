<?php
    ...
    $imgarr = array ();
		if (is_dir ($route . $GLOBALS['imagesfolder'])){
			$mydir = scandir ($route .$GLOBALS['imagesfolder']);
			//Loop through and find any valid images.
			for ($i = 0; $i < count ($mydir); $i++){
				//make sure it's a file
				if (is_file ($route .$GLOBALS['imagesfolder'] . "/" . $mydir[$i])){
					//Check for a valid file type.
					$thepath = pathinfo ($route .$GLOBALS['imagesfolder'] . "/" . $mydir[$i]);
	...
	function createthumb ($img, $mwidth, $mheight, $ext="_th"){
		...
		if (is_file ($route . $GLOBALS['imagesfolder'] . "/" . $img)){
			//Now, get the current file size.
			if ($cursize = getimagesize ($route . $GLOBALS['imagesfolder'] . "/" . $img)){
				...
				$thepath = pathinfo ($route . $GLOBALS['imagesfolder'] . "/" . $img);
				//Setup our thumbnail.
				...
				$src = $types[$thepath['extension']][0] ($route . $GLOBALS['imagesfolder'] . "/" . $img);
				//Create the copy.
				imagecopyresampled ($dst,$src,0,0,0,0,$newsize[0],$newsize[1],$cursize[0],$cursize[1]);
				$retfile[0] = $route . $GLOBALS['thumbsfolder'] . "/" . $filename.$ext.".".$thepath['extension']."";
				$retfile[1] = $filename.$ext.".".$thepath['extension']."";
				
        //Create the thumbnail.
				$types[$thepath['extension']][1] ($dst,$retfile[0]);
				//put in the session
