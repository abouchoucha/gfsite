...
class Team extends Zend_Db_Table_Abstract {
	...
	public function countTeamsByCountry($country, $compId) {
		...
		$sql .= " and s.active = 1 ";
		$sql .= " and lc.sort_priority = " . $typeComp;
	...
	public function findUniqueTeam($teamId) {
		...
		$sql .= " INNER JOIN regiongroup rg ON rg.region_group_id = r.region_id  ";
	...
	public function findPlayersbyTeam($teamId, $userid = null) {
		$db = $this->getAdapter ();
		$sql = " select p.player_id,p.player_firstname, p.player_lastname,p.player_name_short,p.player_common_name,p.player_position,p.player_country,p.player_nationality,p.player_nickname,p.player_seoname,p.player_dob,tp.jersey_number,c.country_name,pimg.imagefilename ";
		if (! is_null ( $userid )) {
			$sql .= " ,(SELECT IF(COUNT(*)=1,'y','n') FROM userplayer WHERE user_id = $userid AND player_id = p.player_id) ismyplayer";
		} else {
			$sql .= ", 'n' as ismyplayer";
		}
		$sql .= " from player p left outer join playerimage pimg on (p.player_id = pimg.player_id)  ";
	...
	public function selectFeaturedTeams($type = null, $limit = null, $regiongroupid = null) {
		$db = $this->getAdapter ();
		$sql = " select t.team_id, t.team_name,t.team_seoname,c.country_id,c.country_name, r.region_name ";
		$sql .= " from featuredteams ft, team t, country c, region r ";
		$sql .= " where ft.team_id = t.team_id ";
		$sql .= " and t.country_id = c.country_id ";
	...
	public function findFeaturedTeams($count) {
		$db = $this->getAdapter ();
		$sql = " select t.team_id, t.team_name,t.team_seoname, c.country_id, c.country_name, r.region_name  ";
		$sql .= " from featuredteams ft, team t, country c,region r ";
		$sql .= " where ft.team_id = t.team_id ";
	...
	public function selectPopularTeams($type) {
		...
		$sql .= " from userteam pt, team t, country c, region r ";
	...
	public function findPopularTeams($count) {
		$db = $this->getAdapter ();
		//$sql = " select t.team_id, t.team_name,t.team_seoname, c.country_name ,team_name_official ";
		//$sql .= " from popularteams pt, team t, country c ";
		//$sql .= " where pt.team_id = t.team_id ";
		//$sql .= " and t.country_id = c.country_id ";
		//$sql .= "  order by pt.sort_order limit 0, " . $count;
	...
	public function findTeamSearch($teamId = null) {
		...
		$sql .= " from team t,country c ";
		$sql .= " where t.country_id = c.country_id ";
		if (! is_null ( $teamId )) {
			$sql .= " and t.team_id =" . $teamId;
		}
