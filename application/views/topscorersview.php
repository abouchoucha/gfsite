	  ...
	  <div class="scores">                        
	   ...
	   <?php $i = 1; ?>
                    ...
                    <?php if($i % 2 == 1) { $style = 'scores1'; }else{ $style = 'scores2';} ?> 
                     ...
                     <?php 
                                $team = new Team();
                                $row = $team->fetchRow($team->select()->where('team_id = ?',  $item['team_id']));
                                //Zend_Debug::dump ($row);
                                ...
                                $player = new Player();
                                $rowplayer = $player->fetchRow($player->select()->where('player_id = ?',  $item['player_id']));
		  ...
		  <div class="<?php echo $style; ?>">                            
		    <ul>                                
				  <li class="name" style="width:120px">                                                                  
						<a href="<?php echo $urlGen->getPlayerMasterProfileUrl(null, null, null, $item["player_id"], true ,$player_common_name); ?>">
