...
class Activity extends Zend_Db_Table_Abstract {
                                                    ...
                                                    )
    {
        ...
        if ($alert != null) {
            ...
            $default_hashtags = "#GoalFace #football #futbol #futebol #soccer";

            // Added to control more than one Competition FB page to write alerts to 
            //$fbpages = array();
            $fbpages = array('id' => $config->facebook->goalface->id, 
                                     'name' => 'Goalface',
            ...
            if ($teamId != null) {
                ...
                if($newArray!=null) {
                    ...
                    echo '<br>Ready to write in TEAM '.$teamId;
                    echo '<br>Write in Facebook TEAM page ';
                    $team = new Team();
            ...
            if ($playerId != null) {
                ...
                foreach ($playerAlertsFaceBook as $alert) {
                    ...
                    if (in_array($activityTypeId, $goalsArray)) {
                        if (in_array(Constants::$_PLAYER_GOAL_ALERTS, $playerFrequencyTypes)) {
                            echo '<br>-->PLAYER GOAL ALERTS-->S ending FACEBOOK for user:' . $alert["user_id"] . ' Frecuency: ' .Constants::$_PLAYER_GOAL_ALERTS." for player:" .$alert['player_id'];
                            //$mail->set_subject($variablesToReplace['player_name'] . ' Goal Alert');
                ...
                // ========================= FACEBOOK ====================================//
                // Write Different Alerts in GOALFACE FB page , World Cup FB page         //
                // =======================================================================//
                ...
                if($newArray!=null){
                    ...
                    foreach ($fbpages as $pages) {
                        ...
                        self::$logger->debug ( "------->Page Name:".$pages['name']);
                        self::$logger->debug ( "------->Page ID:".$pages['id']);
                        self::$logger->debug ( "------->appId:".$app_id);
                        self::$logger->debug ( "------->secret:".$app_secret);
                        ...
                        try{
                            ...
                            self::$logger->debug ( '----*****> Error Page Player(Activity) Alert: No se pudo enviar -> Id_Page:'. $pages['id'] .'Message:'.$e->getMessage());
                            echo ' Code:'.$e->getCode();
                            echo ' Line:'.$e->getLine();
                        }
                    }
