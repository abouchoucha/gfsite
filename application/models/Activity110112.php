                ...
                foreach ($playerAlertsFaceBook as $alert) { //echo " -for each facebook- ";
                    $playerFrequencyTypes = explode(',', $alert['alert_frecuency_type']);
                            ...
                            if ($newArray != null) {
								...
								echo '<br>PosUrl_03'.$variablesToReplace['match_url'];
                                $facebook = new Facebook(array(
                                  'appId'  => $app_id,
                                  'secret' => $app_secret
                                ));
                 ...
                 $newArray = array('typeOfPlayerEvent' => 'started');
					...
					if($newArray!=null){
						...
						if ($row[0]["player_country_extra"] == null) { //added JV 10-12-2012 player for spanish alert have 100 as value
						    $message = $variablesToReplace['player_name'].' '.$newArray['typeOfPlayerEvent'].' the '.$variablesToReplace['match_playing'].' Match. http://www.goalface.com'.$variablesToReplace['match_url'];
						} else {
						    //added JV 10-12-2012
						    $message = $variablesToReplace['player_name'].' '.$newArray['typeOfPlayerEvent'].' en el partido '.$variablesToReplace['match_playing'].'. http://www.goalface.com'.$variablesToReplace['match_url'];
						}
